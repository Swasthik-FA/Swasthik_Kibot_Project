...
**Evidence to commit (in `reviews/g1_specification_lock/`):** completed checklist, signed-off requirements spec, block diagram, preliminary BOM (key components), risk register, gate_log entry.
...
**Evidence to commit (in `reviews/g2_schematic_review/`):** completed checklist, ERC report (clean), schematic PDF export, BOM CSV, findings list with resolution, gate_log entry.
...
**Evidence to commit (in `reviews/g3_layout_review/`):** completed checklist, 3D render, layer-by-layer screenshots of critical areas, plane integrity screenshots with reference plane highlighted for each high-speed net, length matching report (if applicable), thermal calculation, gate_log entry.
...
**Evidence to commit (in `reviews/g4_pre_fab_review/`):** DFM checklist, BOM validation checklist, DRC report (clean), ERC report (clean), fabrication drawing PDF, assembly drawing PDF, pick-and-place CSV, stackup confirmation from fab (if impedance-controlled), gate_log entry.
...
├── reviews/
│   ├── g1_specification_lock/       signed
│   ├── g2_schematic_review/         signed
│   ├── g3_layout_review/            signed
│   ├── g4_pre_fab_review/
